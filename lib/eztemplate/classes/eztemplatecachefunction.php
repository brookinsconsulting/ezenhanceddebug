...
class eZTemplateCacheFunction
{
    ...
    */
            ...
            case $this->BlockName:
            {
                ...
                if ( $ini->variable( 'TemplateSettings', 'TemplateCache' ) != 'enabled' )
                {
                    $text = eZTemplateCacheFunction::processUncached( $tpl, $functionChildren,
                                                                      $rootNamespace, $currentNamespace, $functionParameters );
                    $textElements[] = $text;
                    return;
                }
                else
     ...
     */
    function processUncached( $tpl, $functionChildren, $rootNamespace, $currentNamespace, $functionParameters = false )
    {
        ...
        if( $functionParameters )
        {
            ...
            if( isset( $functionParameters['expiry'] ) )
                $ttl = $functionParameters['expiry'][0][1];

            $text = "TTL : ${ttl} <div class=\"cache-block-border\">${text}</div>";
        }
